...
while [ $limit -ge $i ]
do
    bowtie2 --threads 3 -x $INDEX_DIR/$PRODUCT -1 $FASTQS_DIR/"${Vbarcode[i]}"_R1_.fastq -2 $FASTQS_DIR/"${Vbarcode[i]}"_R2_.fastq -U $FASTQS_DIR/unpaired.fastq -S $SAM_DIR/"${Vbarcode[i]}".sam | samtools view -bS -T $REF_DIR/$FASTA_FILE > $BAM_DIR/"${Vbarcode[i]}".bam
    let i=$i+1
...
#ref_map.pl -o $REFMAP_DIR/P1 -O $LIST_DIR/$POPMAP_FILE -T 4 -X "populations: -m 20 --fasta_loci --fasta_samples_raw --phylip_var --phylip --vcf_haplotypes --genepop --fasta_samples --ordered_export --fasta_samples_raw  --write_random_snp --structure --vcf -p 1 -r 0.75 --fstats -f p_value --bootstrap --verbose -M $LIST_DIR/$POPMAP_FILE -k --smooth_fstats --smooth_popstats --hwe" --samples /$BAM_DIR

#populations -P $REFMAP_DIR/P1 -O $REFMAP_DIR/P2 -M $LIST_DIR/$POPMAP_FILE -m 20 --fasta_samples_raw --phylip_var --phylip --vcf_haplotypes --genepop --fasta_samples --ordered_export --fasta_samples_raw  --write_random_snp --structure --vcf -p 2 -r 0.75 --fstats -f p_value --bootstrap --verbose -k --smooth_fstats --smooth_popstats --hwe --fasta_loci

#populations -P $REFMAP_DIR/P1 -O $REFMAP_DIR/P5 -M $LIST_DIR/$POPMAP_FILE -m 20 --fasta_samples_raw --phylip_var --phylip --vcf_haplotypes --genepop --fasta_samples --ordered_export --fasta_samples_raw  --write_random_snp --structure --vcf -p 5 -r 0.75 --fstats -f p_value --bootstrap --verbose -k --smooth_fstats --smooth_popstats --hwe --fasta_loci
